   ...
   ex: */
      ...
      for(int i = 0; i < c.getWidth(); i++){ //for each column
         if(c.moveArray()[order[i]]){ //if the move is valid
            ...
            //subtracted from total moves -> -value for loss, +value for win
            if (e.gameStatus() == color) return (e.getWidth() * e.getHeight() - e.moveTotal() + 1)/2;
            if (e.gameStatus() == color%2+1) return -(e.getWidth() * e.getHeight() - e.moveTotal() + 1)/2;
         }
      }

      //given that the next move is not a win
      //the upper bound is the longest possible path to victory
      ...
      for(int i = 0; i < c.getWidth(); i++){ //for each column
         ...
         if(c.moveArray()[order[i]]){ //if the move is valid
            ...
            //recurse, switching POV to opponent but keeping leading negative to show it is opponent
            //the negative (minimum) bot's success subtracts from this bot's score
   ...
   output: (hopefully)the best possible move*/
      ...
      for(int i = 0; i < c.getWidth(); i++) {
         ...
         if (c.moveArray()[order[i]]) { //if the move is valid
            ...
            int score = this.negamax(e, (int)Double.NEGATIVE_INFINITY, (int)Double.POSITIVE_INFINITY, this.player, e.moveTotal());

            //if the score is better than the previous best score
            if (score > bestTotal) {
               bestMove = order[i]; //update best move
               bestTotal = score; //update best score
            }
         }
      }
